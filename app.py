...
def make_sale():
    ...
    return render_template('make_sale.html', products=products, payment_methods=payment_methods, receipt=receipt, search=search)
    ...
    if request.method == 'POST':
        ...
        for pid, qty in zip(product_ids, quantities):
            ...
            if product and qty > 0 and product.stock >= qty:
                ...
                db.session.add(sale)
                db.session.commit()  # Commit each sale to get its ID
                sale_ids.append(sale.id)
                items.append({
                    'product': product.name,
                    'quantity': qty,
        ...
        receipt = {
            ...
            'items': items,
            'grand_total': grand_total,
            'sale_id': sale_ids[0] if sale_ids else None  # Use first sale's ID for receipt
        }
        flash('Sale completed successfully!', 'success')
